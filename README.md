...
2026; tables from `scripts/aggregate.py`, charts from `scripts/plot.py`).
...
2-vCPU cap. The throughput charts use a 1,000-block `GetBlockRange` window per request (W = 1000); the
harness also records 100- and 10,000-block windows.

### dense (post-NU5)

![dense — GetBlockRange throughput: Rust scales with concurrency to ~272k blocks/s while Go plateaus near 87k by concurrency 2.](contrib/bench/charts/dense-throughput.svg)

![dense — GetBlock p99 latency (log scale): Rust's tail latency stays far below Go's, which climbs past 25 ms at high concurrency.](contrib/bench/charts/dense-latency.svg)
...
<details>
<summary>Full numbers — GetBlock latency and GetBlockRange throughput</summary>
...
![light — GetBlockRange throughput: Rust holds a ~2–4× lead over Go across the concurrency curve.](contrib/bench/charts/light-throughput.svg)

![light — GetBlock p99 latency (log scale): Rust's tail latency stays below Go's throughout.](contrib/bench/charts/light-latency.svg)
...
- **Fidelity.** 0.05% of requests are cancelled at the `ghz -z` duration boundary; the smaller
  ...
  curve (W ∈ {100, 1000, 10000}); reproduce with `run-bench.sh`, then `aggregate.py` (tables) and
  `plot.py` (charts).
