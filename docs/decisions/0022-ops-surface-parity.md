...
- **gRPC reflection, always on.** `build.rs` now emits a `FileDescriptorSet` via
  ...
  that indirection, reflection is always registered: it is metadata about the API shape, not a
...
- **`--log-level` / `--log-file`.** `--log-level` (default `"info"`) feeds a `tracing_subscriber::EnvFilter`;
  ...
  `create().append()`, so no `tracing-appender` dependency is needed for a single long-lived file handle),
